...
trait HasPublishStatus
{
    ...
    {
        return $this->published?->status ?? null;
    }

    public function setPublishStatus(string $status): void
    {
        if ($this->published) {
            $this->published->update(['status' => $status]);
        } else {
            $this->published()->create(['status' => $status]);
        }
    }
    ...
    {
        return $this->publish_status === PublishStatus::Published->value;
    }
}
